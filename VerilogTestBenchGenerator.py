...
testbenchcode_format = [
    '`include "^vtb_modulename^.v"', 'module ^vtb_modulename^_tb;', '^vtb_inputs^', '^vtb_outputs^', 
...
format_values = [
    ['^vtb_modulename^', ''], ['^vtb_inputs^', ''], ['^vtb_inputsparams^', ''], ['^vtb_outputs^', ''],  
    ['^vtb_outputsparams^', ''], ['^vtb_monitor^', ''], ['^vtb_inputinit^', ''],  ['^vtb_inputchange^', ''], 
...
## Parser Functions
def InputOutputVerilogParser(
    VERILOG_CONTENTS, 
    modulename="",
    timedelay=5, clockdelay=5, 
    nooftestcases=5, alltestcases=False
    ):
    global inputs
    global inputs_withsizes
    global outputs
    ...
    AssignFormatValues(modulename, timedelay, clockdelay, nooftestcases, alltestcases)
    TEST_BENCH_CONTENTS = GenerateTestBench()

    return TEST_BENCH_CONTENTS
